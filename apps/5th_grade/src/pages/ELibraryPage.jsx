...
const newUploads = Array.from({ length: 6 }, (_, index) => books[index % books.length]);
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 sm:gap-4">
                  {newUploads.map((book, index) => (
                    <article className="book-card max-w-[180px] mx-auto w-full" key={`new-${book.title}-${index}`}>
                      ...
                      <p className={`book-title text-sm sm:text-base font-bold mb-1 line-clamp-2 ${isDark ? 'text-[#FFFFFF]' : 'text-gray-800'}`} style={{ fontFamily: 'Poppins, sans-serif' }}>{book.title}</p>
                      <p className={`book-author text-xs sm:text-sm ${isDark ? 'text-[#9CA3AF]' : 'text-gray-500'}`} style={{ fontFamily: 'Poppins, sans-serif' }}>{book.author}</p>
                    </article>
                  ))}
                </div>
              </section>
